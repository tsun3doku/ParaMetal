...
void VoronoiCandidateCompute::dispatch(uint32_t faceCount, uint32_t seedCount) {
    ...
    if (vkAllocateCommandBuffers(vulkanDevice.getDevice(), &allocInfo, &cmd) != VK_SUCCESS) {
        throw std::runtime_error("Failed to allocate VoronoiCandidateCompute command buffer");
    }
    ...
    if (vkCreateFence(vulkanDevice.getDevice(), &fenceInfo, nullptr, &fence) != VK_SUCCESS) {
        ...
        throw std::runtime_error("Failed to create VoronoiCandidateCompute fence");
    }
...
void VoronoiCandidateCompute::createDescriptorSetLayout() {
    ...
    if (vkCreateDescriptorSetLayout(vulkanDevice.getDevice(), &layoutInfo, nullptr, &descriptorSetLayout) != VK_SUCCESS) {
        throw std::runtime_error("Failed to create VoronoiCandidateCompute descriptor set layout");
    }
}

void VoronoiCandidateCompute::createDescriptorPool() {
    ...
    if (vkCreateDescriptorPool(vulkanDevice.getDevice(), &poolInfo, nullptr, &descriptorPool) != VK_SUCCESS) {
        throw std::runtime_error("Failed to create VoronoiCandidateCompute descriptor pool");
    }
}

void VoronoiCandidateCompute::createDescriptorSet() {
    ...
    if (vkAllocateDescriptorSets(vulkanDevice.getDevice(), &allocInfo, &descriptorSet) != VK_SUCCESS) {
        throw std::runtime_error("Failed to allocate VoronoiCandidateCompute descriptor set");
    }
}

void VoronoiCandidateCompute::createPipeline() {
    auto computeShaderCode = readFile("shaders/voronoi_candidates_comp.spv");
    ...
    if (vkCreatePipelineLayout(vulkanDevice.getDevice(), &pipelineLayoutInfo, nullptr, &pipelineLayout) != VK_SUCCESS) {
        throw std::runtime_error("Failed to create VoronoiCandidateCompute pipeline layout");
    }
    ...
    if (vkCreateComputePipelines(vulkanDevice.getDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &pipeline) != VK_SUCCESS) {
        throw std::runtime_error("Failed to create VoronoiCandidateCompute pipeline");
    }
