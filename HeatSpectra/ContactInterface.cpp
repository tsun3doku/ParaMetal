...
ContactInterface::ContactInterface(ResourceManager& resourceManager)
	: resourceManager(resourceManager) {
}


void ContactInterface::mapSurfacePoints(Model& sourceModel, const std::vector<std::unique_ptr<HeatReceiver>>& receivers,
std::vector<std::vector<ContactPairGPU>>& receiverContactPairs,
const Settings& settings) {

	iODT* sourceIodt = resourceManager.getRemesherForModel(&sourceModel);
	if (!sourceIodt) {
		return;
	}
	...
	for (size_t receiverIdx = 0; receiverIdx < receivers.size(); receiverIdx++) {
		...
		iODT* receiverIodt = resourceManager.getRemesherForModel(&receiverModel);
		if (!receiverIodt) {
			continue;
		}
		...
		receiver->uploadContactPairs(receiverContactPairs[receiverIdx]);
	}

	cachedOutlineVertices.clear();
	cachedOutlineVertices.reserve(receivers.size() * 256);
	cachedCorrespondenceVertices.clear();
	cachedCorrespondenceVertices.reserve(receivers.size() * 256);
	...
	for (size_t receiverIdx = 0; receiverIdx < receivers.size(); receiverIdx++) {
		...
		for (size_t triIdx = 0; triIdx < receiverMesh.triangles.size(); triIdx++) {
			...
			l1.position = r1;
			l1.color = patchColor;
			cachedOutlineVertices.push_back(l0);
			cachedOutlineVertices.push_back(l1);
			...
			l3.position = r2;
			l3.color = patchColor;
			cachedOutlineVertices.push_back(l2);
			cachedOutlineVertices.push_back(l3);
			...
			l5.position = r0;
			l5.color = patchColor;
			cachedOutlineVertices.push_back(l4);
			cachedOutlineVertices.push_back(l5);
			...
			for (uint32_t si = 0; si < Quadrature::count; ++si) {
				...
				cachedCorrespondenceVertices.push_back(a);
				cachedCorrespondenceVertices.push_back(b0);
				...
				cachedCorrespondenceVertices.push_back(c0);
				cachedCorrespondenceVertices.push_back(c1);
				...
				cachedCorrespondenceVertices.push_back(d0);
				cachedCorrespondenceVertices.push_back(d1);
			}
		}
	}
}
