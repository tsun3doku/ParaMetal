...
VoronoiSurfaceStage::VoronoiSurfaceStage(const VoronoiStageContext& stageContext)
    : context(stageContext) {
}

bool VoronoiSurfaceStage::createDescriptorPool(uint32_t maxFramesInFlight) {
    (void)maxFramesInFlight;
    const uint32_t maxVoronoiModels = 10;
    const uint32_t totalSets = (maxVoronoiModels * 2);
    ...
    if (vkCreateDescriptorPool(context.vulkanDevice.getDevice(), &poolInfo, nullptr,
        ...
        std::cerr << "[VoronoiSystem] Failed to create surface descriptor pool" << std::endl;
        return false;
    }
    return true;
}

bool VoronoiSurfaceStage::createDescriptorSetLayout() {
    ...
    if (vkCreateDescriptorSetLayout(context.vulkanDevice.getDevice(), &layoutInfo, nullptr,
        ...
        std::cerr << "[VoronoiSystem] Failed to create surface descriptor set layout" << std::endl;
        return false;
    }
    return true;
}

bool VoronoiSurfaceStage::createPipeline() {
    ...
    if (createShaderModule(context.vulkanDevice, computeShaderCode, computeShaderModule) != VK_SUCCESS) {
        std::cerr << "[VoronoiSystem] Failed to create surface compute shader module" << std::endl;
        return false;
    }
    ...
    if (vkCreatePipelineLayout(context.vulkanDevice.getDevice(), &pipelineLayoutInfo, nullptr,
        ...
        std::cerr << "[VoronoiSystem] Failed to create surface pipeline layout" << std::endl;
        return false;
    }
    ...
    if (vkCreateComputePipelines(context.vulkanDevice.getDevice(), VK_NULL_HANDLE, 1,
        ...
        vkDestroyShaderModule(context.vulkanDevice.getDevice(), computeShaderModule, nullptr);
        std::cerr << "[VoronoiSystem] Failed to create surface compute pipeline" << std::endl;
        return false;
    }
