...
void TimingRenderer::createQuadVertexBuffer() {
...
void TimingRenderer::createFontAtlas() {
    int width = 0;
    int height = 0;
    ...
    if (!pixels) {
        std::cerr << "[TimingRenderer] Failed to load font atlas: " << atlasPath << std::endl;
        return;
    }
    ...
    if (vkCreateBuffer(vulkanDevice.getDevice(), &stagingInfo, nullptr, &stagingBuffer) != VK_SUCCESS) {
        ...
        std::cerr << "[TimingRenderer] Failed to create staging buffer" << std::endl;
        return;
    }
    ...
    if (vkAllocateMemory(vulkanDevice.getDevice(), &allocInfo, nullptr, &stagingMemory) != VK_SUCCESS) {
        ...
        stbi_image_free(pixels);
        std::cerr << "[TimingRenderer] Failed to allocate staging memory" << std::endl;
        return;
    }
    ...
    if (vkCreateSampler(vulkanDevice.getDevice(), &samplerInfo, nullptr, &fontSampler) != VK_SUCCESS) {
        std::cerr << "[TimingRenderer] Failed to create font sampler" << std::endl;
        return;
    }
}

void TimingRenderer::createDescriptorPool(uint32_t maxFramesInFlight) {
    ...
    if (vkCreateDescriptorPool(vulkanDevice.getDevice(), &poolInfo, nullptr, &descriptorPool) != VK_SUCCESS) {
        std::cerr << "[TimingRenderer] Failed to create descriptor pool" << std::endl;
        return;
    }
}

void TimingRenderer::createDescriptorSetLayout() {
    ...
    if (vkCreateDescriptorSetLayout(vulkanDevice.getDevice(), &layoutInfo, nullptr, &descriptorSetLayout) != VK_SUCCESS) {
        std::cerr << "[TimingRenderer] Failed to create descriptor set layout" << std::endl;
        return;
    }
}

void TimingRenderer::createDescriptorSets(uint32_t maxFramesInFlight) {
    ...
    if (vkAllocateDescriptorSets(vulkanDevice.getDevice(), &allocInfo, descriptorSets.data()) != VK_SUCCESS) {
        std::cerr << "[TimingRenderer] Failed to allocate descriptor sets" << std::endl;
        return;
    }
...
void TimingRenderer::createPipeline(VkRenderPass renderPass, uint32_t subpassIndex) {
    ...
    if (vkCreatePipelineLayout(vulkanDevice.getDevice(), &layoutInfo, nullptr, &pipelineLayout) != VK_SUCCESS) {
        ...
        std::cerr << "[TimingRenderer] Failed to create pipeline layout" << std::endl;
        return;
    }
    ...
    if (vkCreateGraphicsPipelines(vulkanDevice.getDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &pipeline) != VK_SUCCESS) {
        vkDestroyShaderModule(vulkanDevice.getDevice(), vertShader, nullptr);
        vkDestroyShaderModule(vulkanDevice.getDevice(), fragShader, nullptr);
        std::cerr << "[TimingRenderer] Failed to create pipeline" << std::endl;
        return;
    }
