...
bool ContactLineRenderer::createDescriptorSetLayout() {
    ...
    if (vkCreateDescriptorSetLayout(vulkanDevice.getDevice(), &layoutInfo, nullptr, &descriptorSetLayout) != VK_SUCCESS) {
        std::cerr << "ContactLineRenderer: Failed to create descriptor set layout" << std::endl;
        return false;
    }
...
bool ContactLineRenderer::createDescriptorPool(uint32_t maxFramesInFlight) {
    ...
    if (vkCreateDescriptorPool(vulkanDevice.getDevice(), &poolInfo, nullptr, &descriptorPool) != VK_SUCCESS) {
        std::cerr << "ContactLineRenderer: Failed to create descriptor pool" << std::endl;
        return false;
    }
...
bool ContactLineRenderer::createDescriptorSets(uint32_t maxFramesInFlight) {
    ...
    if (vkAllocateDescriptorSets(vulkanDevice.getDevice(), &allocInfo, descriptorSets.data()) != VK_SUCCESS) {
        ...
        std::cerr << "ContactLineRenderer: Failed to allocate descriptor sets" << std::endl;
        return false;
    }
...
bool ContactLineRenderer::createPipeline(VkRenderPass renderPass, uint32_t subpass) {
    std::vector<char> vertCode;
    std::vector<char> fragCode;
    if (!readFile("shaders/contact_lines_vert.spv", vertCode) ||
        !readFile("shaders/contact_lines_frag.spv", fragCode)) {
        std::cerr << "ContactLineRenderer: Failed to read shader files" << std::endl;
        return false;
    }
    ...
    if (createShaderModule(vulkanDevice, vertCode, vertModule) != VK_SUCCESS ||
        ...
        std::cerr << "ContactLineRenderer: Failed to create shader modules" << std::endl;
        return false;
    }
    ...
    if (vkCreatePipelineLayout(vulkanDevice.getDevice(), &pipelineLayoutInfo, nullptr, &pipelineLayout) != VK_SUCCESS) {
        vkDestroyShaderModule(vulkanDevice.getDevice(), vertModule, nullptr);
        vkDestroyShaderModule(vulkanDevice.getDevice(), fragModule, nullptr);
        std::cerr << "ContactLineRenderer: Failed to create pipeline layout" << std::endl;
        return false;
    }
    ...
    if (vkCreateGraphicsPipelines(vulkanDevice.getDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &pipeline) != VK_SUCCESS) {
        ...
        vkDestroyShaderModule(vulkanDevice.getDevice(), vertModule, nullptr);
        vkDestroyShaderModule(vulkanDevice.getDevice(), fragModule, nullptr);
        std::cerr << "ContactLineRenderer: Failed to create graphics pipeline" << std::endl;
        return false;
    }
...
bool ContactLineRenderer::createOutlinePipeline(VkRenderPass renderPass, uint32_t subpass) {
    std::vector<char> vertCode;
    std::vector<char> fragCode;
    if (!readFile("shaders/contact_lines_vert.spv", vertCode) ||
        !readFile("shaders/contact_lines_frag.spv", fragCode)) {
        std::cerr << "ContactLineRenderer: Failed to read outline shader files" << std::endl;
        return false;
    }
    ...
    if (createShaderModule(vulkanDevice, vertCode, vertModule) != VK_SUCCESS ||
        ...
        std::cerr << "ContactLineRenderer: Failed to create outline shader modules" << std::endl;
        return false;
    }
    ...
    if (vkCreateGraphicsPipelines(vulkanDevice.getDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &outlinePipeline) != VK_SUCCESS) {
        vkDestroyShaderModule(vulkanDevice.getDevice(), vertModule, nullptr);
        vkDestroyShaderModule(vulkanDevice.getDevice(), fragModule, nullptr);
        std::cerr << "ContactLineRenderer: Failed to create outline pipeline" << std::endl;
        return false;
    }
