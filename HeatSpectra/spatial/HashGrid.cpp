...
void HashGrid::initialize(const glm::vec3& minBounds, const glm::vec3& maxBounds, float cellSize, uint32_t maxPointsPerCell) {
    ...
    if (!createBuffers(maxPointsTotal)) {
        std::cerr << "[HashGrid] Failed to create grid buffers" << std::endl;
        cleanupResources();
        return;
    }
    if (!createBuildDescriptorSetLayout()) {
        std::cerr << "[HashGrid] Failed to create descriptor set layout" << std::endl;
        cleanupResources();
        return;
    }
    if (!createBuildDescriptorPool(maxFramesInFlight)) {
        std::cerr << "[HashGrid] Failed to create descriptor pool" << std::endl;
        cleanupResources();
        return;
    }
    if (!createBuildDescriptorSets(maxFramesInFlight)) {
        std::cerr << "[HashGrid] Failed to create descriptor sets" << std::endl;
        cleanupResources();
        return;
    }
    if (!createBuildPipeline()) {
        std::cerr << "[HashGrid] Failed to create build pipeline" << std::endl;
        cleanupResources();
        return;
    }
...
bool HashGrid::createBuffers(uint32_t maxPointsTotal) {
    VkDeviceSize gridBufferSize = sizeof(GridPoint) * maxPointsTotal;
    void* mappedPtr = nullptr;
    if (createStorageBuffer(memoryAllocator, vulkanDevice, nullptr, gridBufferSize,
        gridBuffer, gridBufferOffset_, &mappedPtr, false) != VK_SUCCESS) {
        std::cerr << "[HashGrid] Failed to create grid buffer" << std::endl;
        return false;
    }
    
    VkDeviceSize cellStartBufferSize = sizeof(uint32_t) * params.totalCells;
    if (createStorageBuffer(memoryAllocator, vulkanDevice, nullptr, cellStartBufferSize,
        cellStartBuffer, cellStartBufferOffset_, &mappedPtr, false) != VK_SUCCESS) {
        std::cerr << "[HashGrid] Failed to create cell-start buffer" << std::endl;
        return false;
    }
    
    VkDeviceSize cellCountBufferSize = sizeof(uint32_t) * params.totalCells;
    if (createStorageBuffer(memoryAllocator, vulkanDevice, nullptr, cellCountBufferSize,
        cellCountBuffer, cellCountBufferOffset_, &mappedPtr, false,
        VK_BUFFER_USAGE_TRANSFER_DST_BIT) != VK_SUCCESS) {
        std::cerr << "[HashGrid] Failed to create cell-count buffer" << std::endl;
        return false;
    }
    
    VkDeviceSize paramsBufferSize = sizeof(HashGridParams);
    if (createUniformBuffer(memoryAllocator, vulkanDevice, paramsBufferSize,
        paramsBuffer, paramsBufferOffset_, reinterpret_cast<void**>(&mappedParams)) != VK_SUCCESS) {
        std::cerr << "[HashGrid] Failed to create params buffer" << std::endl;
        return false;
    }
    ...
    for (uint32_t i = 0; i < maxFramesInFlight; ++i) {
        if (createStorageBuffer(memoryAllocator, vulkanDevice, nullptr, occupiedCellsBufferSize,
            occupiedCellsBuffers[i], occupiedCellsBufferOffsets[i], &mappedPtr, false) != VK_SUCCESS) {
            std::cerr << "[HashGrid] Failed to create occupied-cells buffer" << std::endl;
            return false;
        }
        
        if (createStorageBuffer(memoryAllocator, vulkanDevice, nullptr, occupiedCountBufferSize,
            occupiedCountBuffers[i], occupiedCountBufferOffsets[i], &mappedPtr, true,
            VK_BUFFER_USAGE_TRANSFER_DST_BIT) != VK_SUCCESS) {
            std::cerr << "[HashGrid] Failed to create occupied-count buffer" << std::endl;
            return false;
        }
...
void HashGrid::recreateResources(uint32_t maxFramesInFlight) {
    ...
    if (!createBuildDescriptorPool(maxFramesInFlight) ||
        ...
        std::cerr << "[HashGrid] Failed to recreate descriptor resources" << std::endl;
        cleanupResources();
    }
}
...
bool HashGrid::createBuildDescriptorSetLayout() {
...
bool HashGrid::createBuildDescriptorPool(uint32_t maxFramesInFlight) {
...
bool HashGrid::createBuildDescriptorSets(uint32_t maxFramesInFlight) {
...
bool HashGrid::createBuildPipeline() {
    ...
    if (createShaderModule(vulkanDevice, computeShaderCode, computeShaderModule) != VK_SUCCESS) {
        std::cerr << "[HashGrid] Failed to create compute shader module" << std::endl;
        return false;
    }
    ...
    if (vkCreatePipelineLayout(vulkanDevice.getDevice(), &pipelineLayoutInfo, nullptr,
        ...
        std::cerr << "[HashGrid] Failed to create pipeline layout" << std::endl;
        return false;
    }
    ...
    if (vkCreateComputePipelines(vulkanDevice.getDevice(), VK_NULL_HANDLE, 1,
        ...
        vkDestroyPipelineLayout(vulkanDevice.getDevice(), buildPipelineLayout, nullptr);
        buildPipelineLayout = VK_NULL_HANDLE;
        vkDestroyShaderModule(vulkanDevice.getDevice(), computeShaderModule, nullptr);
        std::cerr << "[HashGrid] Failed to create compute pipeline" << std::endl;
        return false;
    }
