...
void OverlayPass::create() {
    ...
    if (!createDepthSampler() ||
        ...
        !createSupportingHalfedgePipeline() ||
        !createIntrinsicNormalsPipeline() ||
        !createIntrinsicVertexNormalsPipeline()) {
        std::cerr << "[OverlayPass] Failed to create render resources" << std::endl;
        destroy();
        return;
    }

    timingOverlay = std::make_unique<TimingRenderer>(
        vulkanDevice,
        maxFramesInFlight,
...
void OverlayPass::updateDescriptors() {
    ...
    for (size_t i = 0; i < maxFramesInFlight; i++) {
        ...
        imageInfos[0].imageView = frameGraphRuntime.getResourceDepthSamplerViews(depthResolveId)[i];
        imageInfos[0].sampler = depthSampler;

        imageInfos[1].imageLayout = VK_IMAGE_LAYOUT_GENERAL;
        imageInfos[1].imageView = frameGraphRuntime.getResourceStencilSamplerViews(depthMsaaId)[i];
...
bool OverlayPass::createOutlineDescriptorPool(uint32_t maxFramesInFlight) {
    ...
    if (vkCreateDescriptorPool(vulkanDevice.getDevice(), &poolInfo, nullptr, &outlineDescriptorPool) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create outline descriptor pool" << std::endl;
        return false;
    }
    return true;
}

bool OverlayPass::createOutlineDescriptorSetLayout() {
    ...
    if (vkCreateDescriptorSetLayout(vulkanDevice.getDevice(), &layoutInfo, nullptr, &outlineDescriptorSetLayout) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create outline descriptor set layout" << std::endl;
        return false;
    }
    return true;
}

bool OverlayPass::createOutlineDescriptorSets(uint32_t maxFramesInFlight) {
    ...
    if (vkAllocateDescriptorSets(vulkanDevice.getDevice(), &allocInfo, outlineDescriptorSets.data()) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to allocate outline descriptor sets" << std::endl;
        return false;
    }
...
bool OverlayPass::createDepthSampler() {
    ...
    if (vkCreateSampler(vulkanDevice.getDevice(), &samplerInfo, nullptr, &depthSampler) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create depth sampler" << std::endl;
        return false;
    }
    return true;
}

bool OverlayPass::createSupportingHalfedgeDescriptorPool(uint32_t maxFramesInFlight) {
    ...
    if (vkCreateDescriptorPool(vulkanDevice.getDevice(), &poolInfo, nullptr, &supportingHalfedgeDescriptorPool) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create supporting halfedge descriptor pool" << std::endl;
        return false;
    }
    return true;
}

bool OverlayPass::createSupportingHalfedgeDescriptorSetLayout() {
    ...
    if (vkCreateDescriptorSetLayout(vulkanDevice.getDevice(), &layoutInfo, nullptr, &supportingHalfedgeDescriptorSetLayout) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create supporting halfedge descriptor set layout" << std::endl;
        return false;
    }
    return true;
}
...
void OverlayPass::allocateDescriptorSetsForModel(Model* model, uint32_t maxFramesInFlight) {
    ...
    if (vkAllocateDescriptorSets(vulkanDevice.getDevice(), &allocInfo, descriptorSets.data()) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to allocate supporting halfedge descriptor sets" << std::endl;
        return;
    }
...
bool OverlayPass::createIntrinsicNormalsDescriptorPool(uint32_t maxFramesInFlight) {
    const uint32_t maxModels = 10;
    ...
    if (vkCreateDescriptorPool(vulkanDevice.getDevice(), &poolInfo, nullptr, &intrinsicNormalsDescriptorPool) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create intrinsic normals descriptor pool" << std::endl;
        return false;
    }
    return true;
}

bool OverlayPass::createIntrinsicNormalsDescriptorSetLayout() {
    std::array<VkDescriptorSetLayoutBinding, 2> bindings{};
    ...
    if (vkCreateDescriptorSetLayout(vulkanDevice.getDevice(), &layoutInfo, nullptr, &intrinsicNormalsDescriptorSetLayout) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create intrinsic normals descriptor set layout" << std::endl;
        return false;
    }
    return true;
}
...
void OverlayPass::allocateNormalsDescriptorSetsForModel(Model* model, uint32_t maxFramesInFlight) {
    ...
    std::vector<VkDescriptorSet> descriptorSets(maxFramesInFlight);
    if (vkAllocateDescriptorSets(vulkanDevice.getDevice(), &allocInfo, descriptorSets.data()) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to allocate intrinsic normals descriptor sets" << std::endl;
        return;
    }
...
bool OverlayPass::createIntrinsicVertexNormalsDescriptorPool(uint32_t maxFramesInFlight) {
    const uint32_t maxModels = 10;
    ...
    if (vkCreateDescriptorPool(vulkanDevice.getDevice(), &poolInfo, nullptr, &intrinsicVertexNormalsDescriptorPool) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create intrinsic vertex normals descriptor pool" << std::endl;
        return false;
    }
    return true;
}

bool OverlayPass::createIntrinsicVertexNormalsDescriptorSetLayout() {
    std::array<VkDescriptorSetLayoutBinding, 2> bindings{};
    ...
    if (vkCreateDescriptorSetLayout(vulkanDevice.getDevice(), &layoutInfo, nullptr, &intrinsicVertexNormalsDescriptorSetLayout) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create intrinsic vertex normals descriptor set layout" << std::endl;
        return false;
    }
    return true;
}
...
void OverlayPass::allocateVertexNormalsDescriptorSetsForModel(Model* model, uint32_t maxFramesInFlight) {
    ...
    std::vector<VkDescriptorSet> descriptorSets(maxFramesInFlight);
    if (vkAllocateDescriptorSets(vulkanDevice.getDevice(), &allocInfo, descriptorSets.data()) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to allocate intrinsic vertex normals descriptor sets" << std::endl;
        return;
    }
...
bool OverlayPass::createOutlinePipeline() {
    ...
    if (createShaderModule(vulkanDevice, vertCode, vertModule) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create outline vertex shader module" << std::endl;
        return false;
    }
    VkShaderModule fragModule = VK_NULL_HANDLE;
    if (createShaderModule(vulkanDevice, fragCode, fragModule) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create outline fragment shader module" << std::endl;
    ...
    if (vkCreatePipelineLayout(vulkanDevice.getDevice(), &pipelineLayoutInfo, nullptr, &outlinePipelineLayout) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create outline pipeline layout" << std::endl;
        vkDestroyShaderModule(vulkanDevice.getDevice(), vertModule, nullptr);
    ...
    if (vkCreateGraphicsPipelines(vulkanDevice.getDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &outlinePipeline) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create outline pipeline" << std::endl;
...
bool OverlayPass::createSupportingHalfedgePipeline() {
    ...
    if (createShaderModule(vulkanDevice, vertShaderCode, vertShaderModule) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create supporting halfedge vertex shader module" << std::endl;
        return false;
    }
    ...
    if (createShaderModule(vulkanDevice, geomShaderCode, geomShaderModule) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create supporting halfedge geometry shader module" << std::endl;
        vkDestroyShaderModule(vulkanDevice.getDevice(), vertShaderModule, nullptr);
        return false;
    }
    VkShaderModule fragShaderModule = VK_NULL_HANDLE;
    if (createShaderModule(vulkanDevice, fragShaderCode, fragShaderModule) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create supporting halfedge fragment shader module" << std::endl;
        vkDestroyShaderModule(vulkanDevice.getDevice(), vertShaderModule, nullptr);
    ...
    if (vkCreatePipelineLayout(vulkanDevice.getDevice(), &pipelineLayoutInfo, nullptr, &supportingHalfedgePipelineLayout) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create supporting halfedge pipeline layout" << std::endl;
        vkDestroyShaderModule(vulkanDevice.getDevice(), vertShaderModule, nullptr);
        vkDestroyShaderModule(vulkanDevice.getDevice(), geomShaderModule, nullptr);
    ...
    if (vkCreateGraphicsPipelines(vulkanDevice.getDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &supportingHalfedgePipeline) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create supporting halfedge pipeline" << std::endl;
...
bool OverlayPass::createIntrinsicNormalsPipeline() {
    ...
    VkShaderModule vertShaderModule = VK_NULL_HANDLE;
    if (createShaderModule(vulkanDevice, vertShaderCode, vertShaderModule) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create intrinsic normals vertex shader module" << std::endl;
        return false;
    }
    VkShaderModule geomShaderModule = VK_NULL_HANDLE;
    if (createShaderModule(vulkanDevice, geomShaderCode, geomShaderModule) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create intrinsic normals geometry shader module" << std::endl;
        vkDestroyShaderModule(vulkanDevice.getDevice(), vertShaderModule, nullptr);
        return false;
    }
    VkShaderModule fragShaderModule = VK_NULL_HANDLE;
    if (createShaderModule(vulkanDevice, fragShaderCode, fragShaderModule) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create intrinsic normals fragment shader module" << std::endl;
        vkDestroyShaderModule(vulkanDevice.getDevice(), vertShaderModule, nullptr);
        vkDestroyShaderModule(vulkanDevice.getDevice(), geomShaderModule, nullptr);
        return false;
    }
    ...
    if (vkCreatePipelineLayout(vulkanDevice.getDevice(), &pipelineLayoutInfo, nullptr, &intrinsicNormalsPipelineLayout) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create intrinsic-normals pipeline layout" << std::endl;
        vkDestroyShaderModule(vulkanDevice.getDevice(), vertShaderModule, nullptr);
        vkDestroyShaderModule(vulkanDevice.getDevice(), geomShaderModule, nullptr);
        vkDestroyShaderModule(vulkanDevice.getDevice(), fragShaderModule, nullptr);
        return false;
    ...
    if (vkCreateGraphicsPipelines(vulkanDevice.getDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &intrinsicNormalsPipeline) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create intrinsic-normals pipeline" << std::endl;
...
bool OverlayPass::createIntrinsicVertexNormalsPipeline() {
    ...
    VkShaderModule vertShaderModule = VK_NULL_HANDLE;
    if (createShaderModule(vulkanDevice, vertShaderCode, vertShaderModule) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create intrinsic vertex normals vertex shader module" << std::endl;
        return false;
    }
    VkShaderModule geomShaderModule = VK_NULL_HANDLE;
    if (createShaderModule(vulkanDevice, geomShaderCode, geomShaderModule) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create intrinsic vertex normals geometry shader module" << std::endl;
        vkDestroyShaderModule(vulkanDevice.getDevice(), vertShaderModule, nullptr);
        return false;
    }
    VkShaderModule fragShaderModule = VK_NULL_HANDLE;
    if (createShaderModule(vulkanDevice, fragShaderCode, fragShaderModule) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create intrinsic vertex normals fragment shader module" << std::endl;
        vkDestroyShaderModule(vulkanDevice.getDevice(), vertShaderModule, nullptr);
        vkDestroyShaderModule(vulkanDevice.getDevice(), geomShaderModule, nullptr);
        return false;
    }
    ...
    if (vkCreatePipelineLayout(vulkanDevice.getDevice(), &pipelineLayoutInfo, nullptr, &intrinsicVertexNormalsPipelineLayout) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create intrinsic vertex normals pipeline layout" << std::endl;
        vkDestroyShaderModule(vulkanDevice.getDevice(), vertShaderModule, nullptr);
        vkDestroyShaderModule(vulkanDevice.getDevice(), geomShaderModule, nullptr);
        vkDestroyShaderModule(vulkanDevice.getDevice(), fragShaderModule, nullptr);
        return false;
    ...
    if (vkCreateGraphicsPipelines(vulkanDevice.getDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &intrinsicVertexNormalsPipeline) != VK_SUCCESS) {
        std::cerr << "[OverlayPass] Failed to create intrinsic vertex normals pipeline" << std::endl;
...
void OverlayPass::record(const FrameContext& context, const SceneView& view, const RenderFlags& flags, const OverlayParams& params, RenderServices& services) {
    ...
    if (params.drawIntrinsicNormals) {
        ...
        for (Model* model : remesher.getRemeshedModels()) {
            ...
            pushConstants.normalLength = params.normalLength;
    ...
    if (params.drawIntrinsicVertexNormals) {
    ...
    if (simulation && flags.drawContactLines && simulation->simulationActive()) {
