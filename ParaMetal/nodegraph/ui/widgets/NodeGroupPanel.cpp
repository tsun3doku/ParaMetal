...
NodeGroupPanel::NodeGroupPanel(QWidget* parent)
    ...
    nodegraphwidgets::styleLineEdit(targetNameLineEdit);
...
void NodeGroupPanel::refreshSourceOptions() {
    if (!sourceTypeComboBox || !sourceNameComboBox || !canEdit()) {
        return;
    }
...
void NodeGroupPanel::refreshFromNode() {
    if (!canEdit()) {
        return;
    }
...
void NodeGroupPanel::applySettings() {
    ...
    if (!NodePanelUtils::writeBoolParam(bridge(), currentNodeId(), nodegraphparams::group::Enabled, enabled) ||
        !NodePanelUtils::writeIntParam(bridge(), currentNodeId(), nodegraphparams::group::SourceType, sourceTypeValue) ||
        !NodePanelUtils::writeStringParam(bridge(), currentNodeId(), nodegraphparams::group::SourceName, sourceName) ||
        !NodePanelUtils::writeStringParam(bridge(), currentNodeId(), nodegraphparams::group::TargetName, targetName)) {
